#!/usr/bin/env bash
#
# Soul of Waifu v2.5.1 — Linux Launcher
...
if command -v ffmpeg &>/dev/null; then
    ...
    echo -e "          Ubuntu/Debian: ${C_WHITE}sudo apt install ffmpeg${C_RESET}"
    echo -e "          Fedora/RHEL:   ${C_WHITE}sudo dnf install ffmpeg${C_RESET}"
    echo -e "          Arch Linux:    ${C_WHITE}sudo pacman -S ffmpeg${C_RESET}"
    echo
    exit 1
fi
echo
...
if [ ! -e "$INSTALL_ENV_DIR/bin/python" ]; then
    ...
    echo -e "        ${C_GOLD}Please run installer.sh first:${C_RESET} ${C_WHITE}bash installer.sh${C_RESET}"
    echo
    exit 1
fi
...
if [ "$EXIT_CODE" -eq 0 ]; then
    ...
    echo -e "     • GPU VRAM Out-of-Memory or backend driver crash."
